...
import { notificationsFor, currentStudent } from "@/lib/data";
...
export default function StudentNotificationsPage() {
  const list = notificationsFor("student");
  ...
  return (
    <PortalShell
      ...
      breadcrumbs={[{ label: "Student", href: "/institution/student" }, { label: "Notifications" }]}
    >
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Notifications</h1>
          <p className="text-sm text-muted-foreground">Stay updated on applications, deadlines and readiness changes.</p>
        </div>
          ...
          {list.map((n) => {
            ...
            return (
              <Card key={n.id} className={cn("flex items-start gap-3 p-4 cursor-pointer", !read && "border-blue/30 bg-blue-light/20")} onClick={() => markNotificationRead(n.id)}>
                <div className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-full", colorMap[n.kind])}>
